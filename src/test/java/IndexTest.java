...
public class IndexTest {
  ...
  public void testParseIndex() {
    ...
    Assert.assertEquals(idx.slices.get(0).url, "test-file.apk.patch1");
    Assert.assertEquals(idx.slices.get(0).checksum, "md5-1");
    Assert.assertEquals(idx.slices.get(0).size, 1);
    Assert.assertNull(idx.slices.get(0).filename);
    ...
    Assert.assertNull(idx.slices.get(1).filename);
